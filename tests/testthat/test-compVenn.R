...
test_that("invalid inputs", {
  ...
  # Test
  expect_error(compVenn(deseq2_result, edger_result[-1, ]),
               "deseq2_result should contain same genes as edger_result.")

  expect_error(compVenn(deseq2_result, edger_result, cutoff = "a"),
               "Cutoff should be a positive number between 0 and 1.")

  expect_error(compVenn(deseq2_result, edger_result, cutoff = -1),
               "Cutoff should be a positive number between 0 and 1.")

  expect_error(compVenn(deseq2_result, edger_result, cutoff = 2),
               "Cutoff should be a positive number between 0 and 1.")

  expect_error(compVenn(deseq2_result, edger_result, filename = 1),
               "Filename should be characters.")

  expect_error(compVenn(edger_result, edger_result),
               "deseq2_result should be a DESeqResults object.")

  expect_error(compVenn(deseq2_result, deseq2_result),
