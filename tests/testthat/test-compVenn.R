...
test_that("invalid inputs", {
  ...
  # Test
  expect_error(compVenn(deseq2Result, edgerResult[-1, ]),
               "deseq2Result should contain same genes as edgerResult.")

  expect_error(compVenn(deseq2Result, edgerResult, cutoff = "a"),
               "Cutoff should be a positive number between 0 and 1.")

  expect_error(compVenn(deseq2Result, edgerResult, cutoff = -1),
               "Cutoff should be a positive number between 0 and 1.")

  expect_error(compVenn(deseq2Result, edgerResult, cutoff = 2),
               "Cutoff should be a positive number between 0 and 1.")

  expect_error(compVenn(deseq2Result, edgerResult, filename = 1),
               "Filename should be characters.")

  expect_error(compVenn(edgerResult, edgerResult),
               "deseq2Result should be a DESeqResults object.")

  expect_error(compVenn(deseq2Result, deseq2Result),
